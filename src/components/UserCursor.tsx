...
function __OriginkitBase_UserCursor(props: Props) {
    ...
    useEffect(() => {
        if (!hideNativeCursor || isStatic || isTouchDevice) return
    ...
    }, [hideNativeCursor, isStatic, isTouchDevice])
    useEffect(() => {
        if (!hideOnTouch) {
...
export default function UserCursor(props: Record<string, unknown>) {
